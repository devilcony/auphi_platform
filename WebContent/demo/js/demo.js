...
Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([rownum,sm, {
		...
		header : '启用状态',
		dataIndex : 'qybz',
		// 演示render的用法(代码转换,该render由<skyform:ext.codeRender/>标签生成)
		renderer : QYBZRender,
		width : 60
	}, {
		header : '剂型',
	...
	store.on('beforeload', function() {
		this.baseParams = {
				xmid: qForm.getForm().findField("query_xmid").getValue(),
				xmmc: qForm.getForm().findField("query_xmmc").getValue(),
				sfdlbm: qForm.getForm().findField("query_sfdlbm").getValue(),
				jx: qForm.getForm().findField("query_jx").getValue(),
				cd: qForm.getForm().findField("query_cd").getValue()
		};
	});
	// 每页显示条数下拉选择框
