...
Ext.onReady(function() {
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'download' && columnIndex == 2) {
			...
			downloadIframe.src = '../upload/download.shtml?fileid=' + fileid;
	...
	var win_swfupload = new Ext.Window({
		...
		items : [ {
			xtype : 'uploadpanel',
			uploadUrl : webContext + '/upload/uploadBasedFlash.shtml',
	...
	function submitTheForm() {
		...
		var requesturl = '../upload/upload.shtml';
		firstForm.form.submit({
			url : '../upload/upload.shtml',
			method : 'POST',
			waitMsg : '正在上传文件,请稍候...',
