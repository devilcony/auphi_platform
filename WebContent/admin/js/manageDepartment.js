...
Ext.onReady(function() {
	...
	var deptTree = new Ext.tree.TreePanel( {
		loader : new Ext.tree.TreeLoader( {
			baseAttrs : {},
			dataUrl : '../organization/departmentTreeInit.shtml'
		}),
		root : root,
		title : '',
		applyTo : 'deptTreeDiv',
	...
	curNode = root;
	curdeptid = deptTree.root.id;
	deptTree.on('click', function(node) {
		Ext.Ajax.request( {
			url : "../organization/view.shtml",
			method : 'POST',
			params : {
				deptId : node.id
			},
			success : function(response) {
				...
				Ext.getCmp("v_deptid").setValue(data.deptid);
				Ext.getCmp("v_deptname").setValue(data.deptname);
				Ext.getCmp("v_parentdeptname").setValue(data.parentdeptname);
				Ext.getCmp("v_customid").setValue(data.customid);
				Ext.getCmp("v_remark").setValue(data.remark);
				Ext.getCmp("v_sortno").setValue(data.sortno);
			}
		});
		curNode = node;
		deptid = node.attributes.id;
		curdeptid = node.attributes.id;
		store.load( {
			params : {
				start : 0,
				limit : bbar.pageSize,
	...
	var store = new Ext.data.Store( {
		proxy : new Ext.data.HttpProxy( {
			url : '../organization/queryDeptsForManage.shtml'
		}),
	...
	var bbar = new Ext.PagingToolbar( {
		...
		var grid = new Ext.grid.GridPanel( {
			title : '<span class="commoncss">部门信息列表</span>',
		...
		qForm = new Ext.form.FormPanel({
			...
			items : [{
				...
				items : [{
							...
							border : false,
							items : [{
								fieldLabel : '上级部门名称', // 标签
								id : 'v_parentdeptname',
								name : 'v_parentdeptname',
								anchor : '80%' // 宽度百分比
							}]
						}]
			}, {
