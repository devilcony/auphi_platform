...
Ext.onReady(function() {
	...
	//var sourceComboData = [ [28, 'mysql' ],[ 31, 'oracle' ], [ 26, 'sql server' ], [ 45, 'hive2' ] ];
	
   /*var dbTypeData = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : '../datasource/getDbTypeList.shtml'
		}),
		autoLoad : true,
		reader : new Ext.data.JsonReader({},['sourceType','sourceTypeName'])		
	});
	
	sourceCombox = new Ext.form.ComboBox( {
		store :dbTypeData,
	...
	function saveParamItem() {
		...
		addParamFormPanel.form.submit({
					url : '../datasource/saveSource.shtml',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
	...
	function update() {
		...
		addParamFormPanel.form.submit({
					url : '../datasource/updateSource.shtml',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function deleteDataSource(){
		...
		Ext.Msg.confirm('请确认', '确认删除选中的数据吗?', function(btn, text) {
			if(btn == 'yes'){
				Ext.Ajax.request({
					url : '../datasource/deleteSource.shtml',
	...
	function testDatabaseParam() {
		addParamFormPanel.form.submit({
					url : '../datasource/testSource.shtml',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
