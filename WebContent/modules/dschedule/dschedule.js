...
function addDependency(){
	...
	if(jobfullname == ''){
		...
		alert(i18n('请选择不同调度'));
		return ;
	} else if (jQuery(('#'+jobfullname+'_'+djobfullname).replace(/\./g,'\\.')).length > 0){
		alert(i18n('依赖关系已存在'));
		return ;
	}
...
function dependencyRow(jobfullname,djobfullname){
	...
	content += '<td>'+jobfullname.substring(1+jobfullname.indexOf('.'),jobfullname.length)+'</td>';
	
	content += '<td>'+djobfullname.substring(1+djobfullname.indexOf('.'),djobfullname.length)+ '</td>' ;
...
function onAddDependency(){
	...
	Ext.Ajax.request({
		...
		success: function(transport) {
			...
			var content = '' ;
			content += '<tr><th width=45%>调度</th><th width=45%>依赖调度</th><th width=10%>操作</th></tr>';
			for(var job in data)
			{
				if(data[job].length == 0)
					content += dependencyRow(job,'') ;
				else
