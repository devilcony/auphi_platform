...
public class DefaultSequenceGenerator implements SequenceGenerator {
	...
	public void init(SystemDao systemDao) {
		initiated = true;
		long initValue = sequenceStorer.load(this.getId(),systemDao);
		...
		sequenceStorer.updateMaxValueByFieldName(maxCount, this.getId(),systemDao);
	}

	public long next(SystemDao systemDao) throws CreateSequnceException {
		if (initiated == false) {
			init(systemDao);
		}
		if (currCount == maxCount) {// 重新申请id
			...
			sequenceStorer.updateMaxValueByFieldName(tmp, this.getId(),systemDao);
			maxCount = tmp;
		}
		currCount++;
		return currCount;
