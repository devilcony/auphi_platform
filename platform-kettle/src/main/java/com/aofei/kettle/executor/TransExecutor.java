...
public class TransExecutor implements Runnable {
    ...
    private TransExecutor(Repository repository,TransExecutionConfiguration transExecutionConfiguration, TransMeta transMeta) {
    ...
    public static synchronized TransExecutor initExecutor(Repository repository, TransExecutionConfiguration transExecutionConfiguration, TransMeta transMeta) {
        TransExecutor transExecutor = new TransExecutor(repository,transExecutionConfiguration, transMeta);
...
//		        	log.logError( trans.getName() + ": preparing transformation execution failed", e );
                ...
                carteObjectId = Trans.sendToSlaveServer( transMeta, executionConfiguration, repository, App.getInstance().getMetaStore() );
        ...
        if(executionConfiguration.isExecutingLocally()) {
            for (StepMetaDataCombi combi : trans.getSteps()) {
                ...
                if(index == null) {
                    ...
                    int errCount = (int) (combi.step.getErrors() + jsonObject.getIntValue("stepStatus"));
                    jsonObject.put("stepStatus", errCount);
                }
            }
        } else if(executionConfiguration.isExecutingRemotely()) {
            ...
            for (int i = 0; i < stepStatusList.size(); i++) {
                ...
                if(index == null) {
                    ...
                    JSONObject jsonObject = jsonArray.getJSONObject(index);
                    int errCount = (int) (stepStatus.getErrors() + jsonObject.getIntValue("stepStatus"));
                    jsonObject.put("stepStatus", errCount);
                }
