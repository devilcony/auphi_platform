package com.aofei.kettle.core.row;
...
public class ValueMetaAndDataCodec {
	...
	public static ValueMetaAndData decode(JSONObject jsonObject) throws KettleValueException {
		...
		String valname = jsonObject.getString("name");
		int valtype = ValueMetaBase.getType(jsonObject.getString("type"));
		String text = jsonObject.getString("text");
		boolean isnull = jsonObject.getBoolean("isnull");
		int len = jsonObject.getInteger("length") == null ? -1 : jsonObject.getInteger("length");
		int prec = jsonObject.getInteger("precision") == null ? -1 : jsonObject.getInteger("precision");
