package com.aofei.kettle.core.base;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.aofei.kettle.App;
import com.aofei.kettle.core.PropsUI;
import com.aofei.kettle.core.cluster.SlaveServerCodec;
...
public abstract class BaseGraphCodec implements GraphCodec {
	...
	public void decodeCommRootAttr(mxCell root, AbstractMeta meta,String repyName) throws KettleException, JsonParseException, JsonMappingException, IOException {
		Repository repository = App.getInstance().getRepository(repyName);
		...
		JSONArray namedParameters = JSONArray.parseArray(root.getAttribute("parameters"));
		for (int i = 0; i < namedParameters.size(); i++) {
			JSONObject jsonObject = namedParameters.getJSONObject(i);

			String paramName = jsonObject.getString("name");
		...
		JSONObject jsonObject = JSONObject.parseObject(root.getAttribute("channelLogTable"));
		ChannelLogTable channelLogTable = meta.getChannelLogTable();
		channelLogTable.setConnectionName(jsonObject.getString("connection"));
		channelLogTable.setSchemaName(jsonObject.getString("schema"));
		channelLogTable.setTableName(jsonObject.getString("table"));
		channelLogTable.setTimeoutInDays(jsonObject.getString("timeout_days"));
		...
		if(jsonArray != null) {
			for ( int i = 0; i < jsonArray.size(); i++ ) {
				...
				if (field != null) {
					field.setFieldName(fieldJson.getString("name"));
					field.setEnabled(fieldJson.getBooleanValue("enabled"));
				}
			}
		}
	}
	...
	public void decodeDatabases(mxCell root, AbstractMeta meta) throws KettleDatabaseException, JsonParseException, JsonMappingException, IOException {
		JSONArray jsonArray = JSONArray.parseArray(root.getAttribute("databases"));
	...
	public void decodeSlaveServers(mxCell root, AbstractMeta meta) throws Exception {
		JSONArray jsonArray = JSONArray.parseArray(root.getAttribute("slaveServers"));
		for (int i = 0; i < jsonArray.size(); i++) {
			JSONObject jsonObject = jsonArray.getJSONObject(i);
