	...
	public void run() {
		try {
			if (executionConfiguration.isExecutingLocally()) {
				...
				try {
					carteObjectId = Trans.sendToSlaveServer( transMeta, executionConfiguration, com.aofei.kettle.App.getInstance().getRepository(), com.aofei.kettle.App.getInstance().getMetaStore() );
	...
	public void capturePreviewData(Trans trans, List<StepMeta> stepMetas) {
		...
		try {
			...
			for (final StepMeta stepMeta : stepMetas) {
				...
				final List<Object[]> prevRowData = new LinkedList<Object[]>();
				final List<Object[]> lastRowData = new LinkedList<Object[]>();
	...
	public JSONObject getPreviewData() {
		...
		for (StepMetaDataCombi combi : trans.getSteps()) {
			...
			if (rowMeta != null) {
				...
				for (int rowNr = 0; rowNr < lastRowData.size(); rowNr++) {
					Object[] rowData = lastRowData.get(rowNr);
					JSONObject row = new JSONObject();
					for (int colNr = 0; colNr < rowMeta.size(); colNr++) {
						String string;
						ValueMetaInterface valueMetaInterface;
